...
import { posToAlgebraic, withTimeout } from '../utils/helpers.js';
...
io.on("connection", (socket) => {
  ...
  socket.on("joinQueue", async (data) => {
    try {
      ...
      // Fetch user's Elo (rank_points)
      const connection = await getConnection();
      const [profiles] = await connection.execute(
        `SELECT rank_points FROM user_profiles WHERE user_id = ?`,
        [userId]
      );
      const elo = profiles.length > 0 ? (profiles[0].rank_points || 500) : 500;
      ...
      // Try to find opponent with similar Elo
  ...
  socket.on("joinGame", async (data) => {
    try {
      ...
      if (room.status === "playing") {
        ...
        // Send existing moves to sync state
        if (room.moves && room.moves.length > 0) {
          socket.emit("syncGameState", {
            moves: room.moves,
            turnNumber: room.moves.length + 1,
          });
        }
  ...
  socket.on("joinSpectator", async (data) => {
    try {
      ...
      if (!room || room.status !== "playing") {
        socket.emit("error", { message: "Trận đấu không tồn tại hoặc đã kết thúc" });
        return;
      }
      ...
      // Join room to receive broadcast events
      socket.join(`room_${roomId}`);
      console.log(`👁️ Spectator joined room_${roomId}`);

      // Gửi toàn bộ trạng thái lịch sử ván cờ cho spectator (nếu có thể lấy từ room.game)
      // Hiện tại ta chỉ gửi gameReady để client load bàn cờ
      const redPlayerName = room.player1.role === "red" ? room.player1.username : room.player2.username;
      const blackPlayerName = room.player1.role === "black" ? room.player1.username : room.player2.username;
      ...
      // Send gameStarted immediately since it's already playing
      socket.emit("gameStarted", {
        roomId,
        matchId: room.matchId,
        redPlayerId: room.redPlayerId,
      ...
      console.error("❌ Error in joinSpectator:", error);
      socket.emit("error", { message: error.message });
    }
  });
  ...
  socket.on("makeMove", async (data) => {
    try {
      ...
      const fromAlgebraic = posToAlgebraic(fromPos);
      const toAlgebraic = posToAlgebraic(toPos);
      
      if (!fromAlgebraic || !toAlgebraic || !room.game) {
        socket.emit("error", { message: "Invalid move format or game state", notYourTurn: true });
        return;
      }

      const move = room.game.move({ from: fromAlgebraic, to: toAlgebraic });
      if (move === null) {
        console.log(`❌ Invalid move by ${playerId}: ${fromPos} -> ${toPos}`);
        socket.emit("error", { message: "Nước đi không hợp lệ (Invalid move)", notYourTurn: true });
        return;
      }
      ...
      // Store move in memory for reconnections/spectators
      if (!room.moves) room.moves = [];
      room.moves.push({ fromPos, toPos, turnNumber: currentTurn + 1, playerId });
