 ...
 */
function findMatch(currentUserId, currentElo = 500) {
  let bestMatch = null;
  let smallestDiff = Infinity;

  for (const [waitingUserId, playerInfo] of waitingPlayers) {
    if (waitingUserId !== currentUserId) {
      const diff = Math.abs(playerInfo.elo - currentElo);
...
async function endMatch(matchId, winnerId, result) {
  ...
  try {
    ...
    // Update player stats using Elo rating system
    const [matchRows] = await connection.execute(
      `SELECT red_player_id, black_player_id FROM matches WHERE match_id = ?`,
      [matchId],
    );

    if (matchRows.length > 0) {
      const redId = matchRows[0].red_player_id;
      const blackId = matchRows[0].black_player_id;

      // Get current Elo (rank_points)
      const [profiles] = await connection.execute(
        `SELECT user_id, rank_points FROM user_profiles WHERE user_id IN (?, ?)`,
        [redId, blackId]
      );

      let redElo = 500, blackElo = 500;
      profiles.forEach(p => {
        if (p.user_id === redId) redElo = p.rank_points || 500;
        if (p.user_id === blackId) blackElo = p.rank_points || 500;
      });

      const kFactor = 32;
      const redExpected = 1 / (1 + Math.pow(10, (blackElo - redElo) / 400));
      const blackExpected = 1 / (1 + Math.pow(10, (redElo - blackElo) / 400));

      let redScore = 0.5, blackScore = 0.5; // default draw
      if (winnerId === redId) { redScore = 1; blackScore = 0; }
      else if (winnerId === blackId) { redScore = 0; blackScore = 1; }

      const newRedElo = Math.max(0, Math.round(redElo + kFactor * (redScore - redExpected)));
      const newBlackElo = Math.max(0, Math.round(blackElo + kFactor * (blackScore - blackExpected)));

      const redStatUpdate = redScore === 1 ? 'wins = wins + 1' : redScore === 0 ? 'losses = losses + 1' : 'draws = draws + 1';
      const blackStatUpdate = blackScore === 1 ? 'wins = wins + 1' : blackScore === 0 ? 'losses = losses + 1' : 'draws = draws + 1';

      await connection.execute(
        `UPDATE user_profiles SET rank_points = ?, ${redStatUpdate} WHERE user_id = ?`,
        [newRedElo, redId]
      );

      await connection.execute(
        `UPDATE user_profiles SET rank_points = ?, ${blackStatUpdate} WHERE user_id = ?`,
        [newBlackElo, blackId]
      );
    }
